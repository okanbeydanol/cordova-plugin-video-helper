...
- (AVMutableVideoComposition *)buildDefaultVideoComposition
{
    ...
    CGFloat rotationInRadians = atan2(transform.b, transform.a);
    CGFloat rotationInDegrees = rotationInRadians * (180.0 / M_PI);
    
    if (rotationInDegrees == 90 || fabs(rotationInDegrees - 90.0) < FLT_EPSILON) {
        CGSize size = videoTrack.naturalSize;
        videoComposition.renderSize = CGSizeMake(size.height, size.width);
        CGAffineTransform rotation = CGAffineTransformMake(0, 1.0, -1.0, 0, 0, 0);
        CGAffineTransform combinedTransform = CGAffineTransformConcat(rotation, CGAffineTransformMakeTranslation(size.height, 0));
        [passThroughLayer setTransform:combinedTransform atTime:kCMTimeZero];
    }
    else if (rotationInDegrees == -90 || fabs(rotationInDegrees + 90.0) < FLT_EPSILON) {
        CGSize size = videoTrack.naturalSize;
        videoComposition.renderSize = CGSizeMake(size.height, size.width);
        CGAffineTransform rotation = CGAffineTransformMake(0, -1.0, 1.0, 0, 0, 0);
        CGAffineTransform combinedTransform = CGAffineTransformConcat(rotation, CGAffineTransformMakeTranslation(0, size.width));
        [passThroughLayer setTransform:combinedTransform atTime:kCMTimeZero];
    }
    else {
